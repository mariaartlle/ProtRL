...
def score_interface(pdb_file, binder_chain="B"):
    ...
    interface_residues_set, target_contacts = hotspot_residues(pdb_file, binder_chain)

    interface_residues_pdb_ids = []
    interface_residues_pdb_ids_target = []
    ...
    # Same, but for the target chain (the lenght and the residues are always the same)
    for pdb_res_num, aa_type in target_contacts.items():
        interface_AA[aa_type] += 1
        interface_residues_pdb_ids_target.append(f"{target_chain}{pdb_res_num}")

    interface_residues_pdb_ids_str = ','.join(interface_residues_pdb_ids)
    interface_residues_pdb_ids_target_str = ','.join(interface_residues_pdb_ids_target)
