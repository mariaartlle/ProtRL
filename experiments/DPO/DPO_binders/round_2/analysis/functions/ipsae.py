...
pae_cutoff =       float(sys.argv[2])
dist_cutoff =      float(sys.argv[3])
...
# Look for the "full_data" JSON file (this is your main file with all data)
full_data_files = [f for f in json_files if "confidences" in os.path.basename(f) and "summary" not in os.path.basename(f)]
json_file_path = full_data_files[0]
print("Using full_data JSON file:", json_file_path)

json_summary_file_path = json_file_path.replace("confidences","summary_confidences")
print("Using summary JSON file:", json_summary_file_path)

pdb_path = json_file_path.replace("confidences","model").replace("json","cif")


print("Using CIF file:", pdb_path)
pdb_stem = pdb_path.replace(".cif", "")
path_stem = f'{pdb_stem}_{pae_string}_{dist_string}'
af2 = False
af3 = True

file_path = folder + path_stem + ".txt"
file2_path = folder + path_stem + "_byres.txt"
pml_path = folder + path_stem + ".pml"

os.makedirs("ipsae_results" + path_stem + "_ipsae", exist_ok=True) 

OUT = open(path_stem + "_ipsae/" + file_path, 'w')
OUT2 = open(path_stem + "_ipsae/" + file2_path, 'w')
PML = open(path_stem + "_ipsae/" + pml_path, 'w')
...
OUT.write("Chn1,Chn2,PAE,Dist,Type,ipSAE,ipSAE_d0chn,ipSAE_d0dom,ipTM_af,ipTM_d0chn,n0res,n0chn,n0dom,d0res,d0chn,d0dom,nres1,nres2,dist1,dist2,Model\n")
PML.write("Chn1,Chn2,PAE,Dist,Type,ipSAE,ipSAE_d0chn,ipSAE_d0dom,ipTM_af,ipTM_d0chn,n0res,n0chn,n0dom,d0res,d0chn,d0dom,nres1nres2,dist1,dist2,Model\n")
