...
def hotspot_residues(trajectory_pdb, binder_chain="B", atom_distance_cutoff=4.0):
    ...
    for binder_idx, close_indices in enumerate(pairs):
        try:
            ...
            binder_id = binder_residue.id[1]

            aa_single_letter = three_to_one_map[binder_resname]
            binder_interactions[binder_id] = aa_single_letter

            # Process target atoms interacting with this binder atom
            for target_idx in close_indices:
                target_residue = target_atoms[target_idx].get_parent()
                target_resname = target_residue.get_resname()
                target_id = target_residue.id[1]

                target_aa = three_to_one_map[target_resname]
                target_interactions[target_id] = target_aa
        except:
            print("")

    return binder_interactions, target_interactions
...
def calc_ss_percentage(pdb_file, advanced_settings, chain_id="B", atom_distance_cutoff=4.0):
    ...
    interacting_residues, target_contacts = hotspot_residues(pdb_file, chain_id, atom_distance_cutoff)
